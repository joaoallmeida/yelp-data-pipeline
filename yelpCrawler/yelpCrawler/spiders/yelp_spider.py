...
class YelpSpider(scrapy.Spider):
    ...
    def menu_items(self, response: Response):
        category = response.xpath('//*[@id="search_description"]/@value').get()
        page = response.xpath('//div[contains(@class,"pagination-link-current")]//text()').get()
        ...
        for content in response.xpath('//div[contains(@class,"businessName")]//h3'):
            ...
            yield response.follow(content_url, self.content ,cb_kwargs={ "rank": rank, "category": category, "page": page })
    ...
    def content(self, response: Response, rank, category, page):
        ...
        item['title'] = response.xpath('//div[contains(@class,"photo-header-content")]//h1/text()').get()
        item['address'] = ' '.join(response.xpath('//*[@id="main-content"]//section//address//text()').getall())

        if item['title'] is None:
            item['title'] = response.xpath('//*[@id="main-content"]//h1/text()').get()

        if item['address'] is None:
            item['address'] = ' '.join(response.xpath('//*[@id="location-and-hours"]//section/div[2]/div[1]//text()').getall())
